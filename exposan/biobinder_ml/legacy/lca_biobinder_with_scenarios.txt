...
def create_system(
    ...
    # limit_internal_recovery_to_coldest_allowed(HTL, coldest_id='cooling_water')
    ...
    BiocrudeSplitter = u.BiocrudeSplitter(
        ...
        # cutoff_fracs=[crude_fracs[0], crude_fracs[1]*oil_fracs[0], crude_fracs[1]*oil_fracs[1]],
    ...
    #     CrudeHeavyDis,
    #     Lr_trial_range=Lr_trial_range,
    #     Hr_trial_range=Hr_trial_range,
    # )
    ...
    #     'DilutionTank',
    #     ins=liquids_to_disposal_lst,
    #     tau=24,  #hours
    #     outs='hydroponic_water'
    # )

    # def adjust_prices_and_water_flow():
    #     FeedstockTrans._run()        
    #     # Centralized HTL and upgrading, transport feedstock
    #     if decentralized_HTL is False:
    #         dw_price = price_dct['trans_feedstock'] # $/dry mass
    ...
    # DilutionTank.add_specification(adjust_prices_and_water_flow)
...
# Load impact indicators and items
    ...
    def register_impact_indicators():
      """
       Create all TRACI 2.1 ImpactIndicators only once.
       Returns a dict of ImpactIndicators.
      """
      indicators = {}
      for name, meta in TRACI_CATEGORIES.items():
           indicators[name] = qs.ImpactIndicator(
            name,
    ...
    def load_cf_excel():
        """
       Load Excel containing multiple sheets, one per TRACI category.
       Each sheet must contain:
       - ID
       - expected
       The sheet name must match a TRACI_CATEGORIES key.
    
        Returns:
        cf_dict[item_ID][impact_category] = expected_value
        """
        cf_dict = { }
        path = r"C:\Work\Rutgers\QSDsan\EXPOsan\exposan\biobinder_ml\data\impact_items_biobinder.xlsx"
        xls = pd.ExcelFile(path)

        for cat in TRACI_CATEGORIES.keys():
            if cat not in xls.sheet_names:
                print(f"[Warning] No sheet named {cat}, skipping.")
                continue

            df = pd.read_excel(xls, sheet_name=cat)

            if 'ID' not in df or 'expected' not in df:
                raise ValueError(f"Sheet {cat} missing ID or expected column.")

            for _, row in df.iterrows():
                excel_id = str(row['ID']).strip()

                if excel_id not in cf_dict:
                  cf_dict[excel_id] = {}

                cf_dict[excel_id][cat] = float(row['expected'])

        return cf_dict

    ITEM_MAPPING = {
    'feedstock':               'feedstock_item',
    'landfill':                'landfill_item',  # if needed
    'trans_feedstock':         'feedstock_trans_surrogate_item',
    'trans_biocrude':          'biocrude_trans_surrogate_item',
    'natural_gas':             'natural_gas_item',
    'process_water':           'scaled_process_water_item',
    'electricity':             'e_item',
    'steam':                   'steam_item',
    'cooling_water':           'cooling_water_item',
    'chilled_water':           'chilled_water_item',
    'cooling':                 'cooling_item',
    'diesel':                  'biofuel_item',
    'wastewater':              'ww_to_disposal_item'
     }
    def make_lca_item(excel_id, linked_stream, cf_dict):
       """
       excel_id: ID from Excel (e.g., 'feedstock', 'cooling', etc.)
       linked_stream: QSDsan stream (StreamImpactItem) or None (ImpactItem)
       """

       
       item_id = ITEM_MAPPING.get(excel_id, excel_id)

       # Load CFs for all TRACI categories
       kwargs = {
          cat: cf_dict.get(excel_id, {}).get(cat, 0.0)
           for cat in TRACI_CATEGORIES.keys()
        }

       # Create the appropriate QSDsan object
       if linked_stream is None:
          return qs.ImpactItem(ID=item_id, **kwargs)
       else:
          return qs.StreamImpactItem(ID=item_id, linked_stream=linked_stream, **kwargs)
      
    register_impact_indicators()
    cf_dict = load_cf_excel()
    
    def apply_biofuel_zero_CF(cf_dict):
        """
        Set ALL TRACI CFs to zero for ALL biofuel-related items.
        """
        if 'diesel' in cf_dict:
            for cat in TRACI_CATEGORIES.keys():
                cf_dict['diesel'][cat] = 0.0
            return cf_dict
        
    if scenario_biofuel_zero_CF:
       cf_dict = apply_biofuel_zero_CF(cf_dict)
    
    def apply_landfill_zero_CF(cf_dict):
       """
       Return a new cf_dict where landfill CFs are forced to 0
       across all TRACI categories.
       """
       if 'landfill' in cf_dict:
           for cat in TRACI_CATEGORIES.keys():
               cf_dict['landfill'][cat] = 0.0
           return cf_dict
    if scenario_landfill_zero_CF:
        cf_dict = apply_landfill_zero_CF(cf_dict)
       
    feedstock_item = make_lca_item('feedstock', scaled_feedstock, cf_dict)
    landfill__item = make_lca_item('landfill', scaled_feedstock, cf_dict)
    trans_feedstock_item = make_lca_item('trans_feedstock', FeedstockTrans.ins[1], cf_dict)
    process_water_item = make_lca_item('process_water', ProcessWaterScaler.ins[0], cf_dict)
    trans_biocrude_item = make_lca_item('trans_biocrude', BiocrudeTrans.ins[1], cf_dict)
    natural_gas_item = make_lca_item('natural_gas', natural_gas, cf_dict)
    ww_to_disposal_item = make_lca_item('wastewater', ww_to_disposal, cf_dict)
    solids_to_disposal_item = make_lca_item('trans_feedstock', solids_to_disposal, cf_dict)
    biofuel_item = make_lca_item('diesel', biofuel, cf_dict)
    e_item = make_lca_item('electricity', None, cf_dict)
    steam_item = make_lca_item('steam', None, cf_dict)
    cooling_item = make_lca_item('cooling', None, cf_dict)
    ...
    nothing_item = make_lca_item('nothing', fake_stream, cf_dict)
...
def simulate_and_print(sys, save_report=False):
    ...
    scaled_feedstock=sys.flowsheet.stream.scaled_feedstock
    ...
    print(f"OPEX intensity: ${opex_per_ton:.2f} per ton-feed")
    print(f"CAPEX intensity: ${capex_per_ton:.2f} per ton-feed")
        
    all_impacts = lca.get_allocated_impacts(streams=(biobinder,), operation_only=True, annual=True)
    annual_mass = biobinder.F_mass * lca.system.operating_hours
    for cat in TRACI_CATEGORIES.keys():
        print(cat, all_impacts.get(cat, 0) / annual_mass)
...
# def sweep_tipping_fee(sys, price_dct, factor=1.0, start=-80, stop=40, step=20):
#     tea = sys.TEA
#     lca = sys.LCA
#     biobinder = sys.flowsheet.stream.biobinder
#     results = []
    
#     for tipping_fee in np.arange(start, stop + step, step):
#         price_dct['tipping'] = tipping_fee / 1e3 * factor
#         try:
#             # Reset the flowsheet (not the system)
#             sys.flowsheet.clear()
            
#             # Rebuild and simulate
#             sys.simulate()
...
#             print(f"Tipping = {tipping_fee:>5.1f} $/ton → IRR = {IRR*100:>5.2f}%, MSP = ${MSP:>5.2f}/kg, GWP = {GWP:>6.3f} kg CO2e/kg")
#             results.append((tipping_fee, IRR * 100, MSP, GWP))
#         except Exception as e:
#             print(f"Tipping {tipping_fee} failed: {e}")
#             results.append((tipping_fee, None, None, None))
...
if __name__ == '__main__':
    ...
    # CrudeHeavyDis = sys.flowsheet.unit.CrudeHeavyDis
    # print("Used Lr/Hr:", CrudeHeavyDis.Lr, CrudeHeavyDis.Hr)
    # sweep_tipping_fee(sys,  price_dct, factor=SnowdenSwan_factor)