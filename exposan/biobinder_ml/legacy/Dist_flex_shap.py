...
def create_system(
        ...
        crude_fracs=None,
        oil_fracs=None,
        process_conditions=None,        # dict of the DOE row
    ...
    dry_mass = (
    feedstock_composition["Carbohydrates"]
  + feedstock_composition["Proteins"]
  + feedstock_composition["Lipids"]
  + feedstock_composition["Ash"]
     )
    carb_wt    = feedstock_composition["Carbohydrates"] / dry_mass * 100
    protein_wt = feedstock_composition["Proteins"]      / dry_mass * 100
    lipids_wt  = feedstock_composition["Lipids"]        / dry_mass * 100
    ash_wt     = feedstock_composition["Ash"]           / dry_mass * 100
    ...
    if process_conditions is None:
       process_conditions = _default_process
    else:
       process_conditions = dict(process_conditions)
    fd = FeatureDefaults()
    process_for_rf = {
    "Temperature (C)": float(process_conditions.get("Temperature (C)", _default_process["Temperature (C)"])),
    "Residence Time": float(process_conditions.get("Residence Time", _default_process["Residence Time"])),
    "Solid content (w/w) %": float(process_conditions.get("Solid content (w/w) %", _default_process["Solid content (w/w) %"])),
    "Pre-processing": int(process_conditions.get("Pre-processing", fd.pre_processing)),
    "Catalyst": int(process_conditions.get("Catalyst", fd.catalyst)),
    "Reactor Type": int(process_conditions.get("Reactor Type", fd.reactor_type)),
    "Solvent": int(process_conditions.get("Solvent", fd.solvent)),
    "Reactor Volume (mL)": float(process_conditions.get("Reactor Volume (mL)", fd.reactor_volume_ml)),
     }
    if HTL_yields_override is not None:
        _y = dict(HTL_yields_override)
    else:
        _y = predict_htl_yields_from_rf(
       rf_model=rf_model,
       carb_wt=carb_wt,
       protein_wt=protein_wt,
       lipids_wt=lipids_wt,
       ash_wt=ash_wt,
       process=process_for_rf,
       )
    s = sum(_y.values())
    if not np.isfinite(s) or s <= 0:
        raise ValueError(f"Non-physical HTL yields: {_y}")
    _y = {k: float(v)/s for k, v in _y.items()}
    
    HTL_yields.update(_y)
    print("RF/override HTL yields used:")
    for k, v in HTL_yields.items():
        print(f"  {k}: {v:.3f}")
    print("Process used for RF yields:")
    print({k: process_for_rf[k] for k in [
    "Temperature (C)", "Residence Time", "Solid content (w/w) %",
    "Pre-processing", "Catalyst", "Reactor Type", "Solvent", "Reactor Volume (mL)"
     ]})
