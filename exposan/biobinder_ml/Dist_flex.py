...
__all__ = ('create_system', 'gwp_dict')
...
def create_system(
    ...
    try:
        ...
        raise KeyError(f"No feedstock price defined for '{feedstock_id}'")
  ...
  #   rf_pred = predict_htl_yields_from_rf(
  #   rf_model=rf_model,
  #   carb_wt=carb_wt,
  ...
  #    )
    ...
    CrudeHeavyDis = qsu.ShortcutColumn(
        ...
        LHK= ('4M-PHYNO','INDOLE'), #BiocrudeSplitter.keys[1]
        P=50*_psi_to_Pa,
    ...
    # target_fuel_share = cutoff_fracs[1] / (cutoff_fracs[1] + cutoff_fracs[2])
...
def get_EBITDA(tea):
    """
    Return EBITDA [USD/yr] for a BioSTEAM/QSDsan TEA object.
    """
    return tea.sales - tea.AOC


def get_EBITDA_margin(tea):
    sales = tea.sales
    return get_EBITDA(tea) / sales if sales else np.nan


def get_EBITDA_over_TCI(tea):
    TCI = tea.TCI
    return get_EBITDA(tea) / TCI if TCI else np.nan
...
def simulate_and_print(sys, save_report=False):
    ...
    EBITDA_margin = get_EBITDA_margin(tea)
    EBITDA_over_TCI = get_EBITDA_over_TCI(tea) 
    print(f"OPEX intensity: ${opex_per_ton:.2f} per ton-feed")
    print(f"CAPEX intensity: ${capex_per_ton:.2f} per ton-feed")
    print(f"EBITDA: ${EBITDA/1e6:.2f} million/yr")
    print(f"EBITDA margin: {EBITDA_margin:.2%}")
    print(f"EBITDA / TCI: {EBITDA_over_TCI:.2%}")
...
if __name__ == '__main__':
    ...
    print("Used Lr, Hr, LHK:", CrudeHeavyDis.Lr, CrudeHeavyDis.Hr, CrudeHeavyDis.LHK)
