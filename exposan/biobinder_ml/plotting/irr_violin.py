# -*- coding: utf-8 -*-
"""
Created on Tue Mar 10 13:53:15 2026
...
base_positions = np.arange(len(feedstock_order)) * 1.75
...
for body, (_, cfg, prod, _) in zip(vp["bodies"], meta):
    c = colors[(cfg, prod)]
...
# Optional subtle separators between feedstocks
for x in (base_positions[:-1] + base_positions[1:]) / 2:
    ax.axvline(x, color="grey", linewidth=0.8, alpha=0.12)
...
# Save
# ==============================
plt.savefig("IRR_violin_box_swarm_2.png", dpi=300, bbox_inches="tight")
plt.savefig("IRR_violin_box_swarm_2.pdf", bbox_inches="tight")
plt.show()