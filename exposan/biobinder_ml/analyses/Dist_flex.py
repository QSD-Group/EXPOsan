...
from exposan.biobinder_ml._process_settings import set_dynamic_LHK_keys
...
def create_system(
    ...
    # def set_dynamic_LHK_keys():
    #     s = BiocrudeSplitter.outs[0]
    #     cmps = s.chemicals
    #     masses = s.imass
    #     Tbs = {ID: cmps[ID].Tb for ID in cmps.IDs if cmps[ID].Tb is not None}

    #     # Sort by Tb (ascending), break ties by mass (descending)
    #     sorted_IDs = sorted(Tbs, key=lambda ID: (Tbs[ID], -masses[ID]))
    #     nonzero_IDs = [ID for ID in sorted_IDs if masses[ID] > 0]

    #     if len(nonzero_IDs) < 3:
    #         raise RuntimeError("Not enough components with mass and boiling points for two LHK pairs.")

    #     # Store separate keys for the two columns
    #     BiocrudeSplitter.LHK_light = (nonzero_IDs[0], nonzero_IDs[1])  # light vs medium/heavy
    #     BiocrudeSplitter.LHK_heavy = (nonzero_IDs[1], nonzero_IDs[2])  # medium vs heavy

    #     print(f"[INFO] CrudeLightDis LHK: {BiocrudeSplitter.LHK_light}")
    #     print(f"[INFO] CrudeHeavyDis LHK: {BiocrudeSplitter.LHK_heavy}")
    
    # BiocrudeSplitter.add_specification(set_dynamic_LHK_keys)
    # BiocrudeSplitter.run_after_specifications = True
    ...
    CrudeLightDis = qsu.ShortcutColumn(
        ...
        LHK=('ETHYLBEN', '4M-PHYNO'),
        P=50*_psi_to_Pa,
        Lr=0.87,
        Hr=0.98,
        k=2, is_divided=True)
    
    CrudeLightDis.register_alias('CrudeLightDis')
    ...
    CrudeHeavyDis = qsu.ShortcutColumn(
        ...
        LHK=('7MINDOLE', 'C16:0FA'),
        P=50*_psi_to_Pa,
        # Lr=0.97, # 0.57, 0.65, 0.97
        # Hr=0.89,
        y_top=0.90,
        x_bot=0.10,
    ...
    CrudeHeavyDis.register_alias('CrudeHeavyDis')
...
def simulate_and_print(sys, save_report=False):
    ...
    col_light = sys.flowsheet.unit.CrudeLightDis
    col_heavy = sys.flowsheet.unit.CrudeHeavyDis
    
    set_dynamic_LHK_keys(splitter, col_light, col_heavy)
    
    
    # CrudeLightDis.LHK = splitter.LHK_light
    # CrudeHeavyDis.LHK = splitter.LHK_heavy
    sys.simulate()
