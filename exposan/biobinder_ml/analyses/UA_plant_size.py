# -*- coding: utf-8 -*-
"""
Created on Thu May 22 14:18:37 2025
...
plant_df = pd.read_excel(os.path.join(output_dir, "simulation_results_05222025.xlsx"))
valid_configs = ['CHCU_no_EC', 'CHCU_EC_Futures', 'DHCU_no_EC', 'DHCU_EC_Futures']
config_to_sizes = (
    plant_df[plant_df['Configuration'].isin(valid_configs)]
    .groupby('Configuration')['Plant Size (tpd)']
    .apply(list)
    .to_dict()
)
...
if __name__ == "__main__":               
    ...
    for config_name, plant_sizes in config_to_sizes.items():
        print(f"\n🔧 Configuration: {config_name}")

        for size in plant_sizes:
            print(f"📏 Plant size: {size} tpd")
            config = next(c for c in config_dict if c["name"] == config_name)

            config_kwargs = dict(
            flowsheet=None,
            central_dry_flowrate=size * 907.185 / (24 * 0.9),  # Convert tpd to kg/hr assuming 90% uptime
            pilot_dry_flowrate=None,
            **{k: v for k, v in config.items() if k != "name"},
        )

            model = BiobinderModel(config_kwargs=config_kwargs)
            ...
            df = model.run_uncertainty_analysis(n_samples=1000)
            file_path = os.path.join(output_dir, f"UA_{config_name}_Size{int(size)}_{timestamp}.xlsx")
