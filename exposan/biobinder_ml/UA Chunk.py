# -*- coding: utf-8 -*-
"""
Created on Fri Mar 27 17:25:14 2026
...
FEEDSTOCKS = ["manure","green"] #"fog", "sludge", "manure","food","green"
...
def main():
    # -----------------------------
    # CHUNK / PARALLEL SETTINGS
    # -----------------------------
    MASTER_N_SAMPLES = N_SAMPLES      # e.g. 10000
    CHUNK_SIZE = 1000                 # e.g. 1000
    CHUNK_ID = 9                      # set 0..9 for 10 chunks
    N_WORKERS = 6                     # keep modest for green/manure CHCU
    AUTOSAVE_EVERY = 25
    SKIP_IF_OUTPUT_EXISTS = True

    # Known hard-crash samples to skip up front
    SKIP_SAMPLE_IDS = {
        ("manure", "CHCU_No_EC"): {2209, 2210, 2211},
        ("green", "CHCU_No_EC"):  {3908, 3909, 3910},
    }

    if MASTER_N_SAMPLES % CHUNK_SIZE != 0:
        raise ValueError(
            f"MASTER_N_SAMPLES ({MASTER_N_SAMPLES}) must be divisible by CHUNK_SIZE ({CHUNK_SIZE})."
        )

    N_CHUNKS = MASTER_N_SAMPLES // CHUNK_SIZE
    if not (0 <= CHUNK_ID < N_CHUNKS):
        raise ValueError(f"CHUNK_ID must be between 0 and {N_CHUNKS - 1}, got {CHUNK_ID}.")
    ...
    for feedstock_id in FEEDSTOCKS:
        for cfg in CONFIGS:
            ...
            chunk_tag = f"chunk-{CHUNK_ID+1:02d}-of-{N_CHUNKS:02d}"
            meta_path = os.path.join(
                OUTPUT_DIR,
                f"META_SHAP3_{MASTER_N_SAMPLES}_{feedstock_id}_{config_name}_prefer-{PREFER}_{chunk_tag}_{run_ts}.xlsx",
            )

            # Skip if chunk already finished
            if SKIP_IF_OUTPUT_EXISTS:
                prefix = f"META_SHAP3_{MASTER_N_SAMPLES}_{feedstock_id}_{config_name}_prefer-{PREFER}_{chunk_tag}_"
                existing = [
                    f for f in os.listdir(OUTPUT_DIR)
                    if f.startswith(prefix) and f.endswith(".xlsx")
                ]
                if existing:
                    print(f"⏭️ Chunk already exists, skipping: {existing[-1]}")
                    continue

            # ---------- BUILD FULL SAMPLE SET, THEN SLICE THIS CHUNK ----------
            ...
            feature_names, samples = generate_lhs_samples(spec, MASTER_N_SAMPLES, seed=SEED)
            ...
            print(
                f"🧩 Running {chunk_tag} | "
                f"global samples {start_idx+1}-{end_idx} of {MASTER_N_SAMPLES}"
            )

            # ---------- BUILD TASKS ----------
            results = []  # initialize before task creation so skipped rows are recorded here
            tasks = []

            skip_ids = SKIP_SAMPLE_IDS.get((feedstock_id, config_name), set())

            for global_i, sample in enumerate(chunk_samples, start=start_idx + 1):
                if global_i in skip_ids:
                    print(f"⏭️ Skipping known crash sample {global_i} for {feedstock_id} | {config_name}")
                    results.append(
                        {
                            "Sample_ID": global_i,
                            ...
                            "OK": False,
                            "Error": "known_skipped_hard_crash_sample",
                        }
                    )
                    continue

                sample = dict(sample)
                sample["Sample_ID"] = global_i
            ...
            ok = 0
            fail = sum(1 for r in results if not r.get("OK", False))  # count skipped rows as failed

            print(
                f"🚀 Launching {len(tasks)} simulations with {N_WORKERS} workers... "
                f"(pre-skipped: {len(results)})"
            )
            ...
            with cf.ProcessPoolExecutor(max_workers=N_WORKERS, mp_context=ctx) as ex:
                ...
                for fut in cf.as_completed(future_to_sid):
                    ...
                    if done % AUTOSAVE_EVERY == 0:
                        ...
                        df_partial.sort_values("Sample_ID", inplace=True)
                        df_partial.to_excel(meta_path, index=False)
                        print(f"💾 Autosaved progress at sample {sid} -> {meta_path}")

            if pool_broken:
                print("⚠️ Parallel pool broke. Partial results for this chunk will be saved.")
    ...
    #                     data=X_ok.iloc[i],
    #                     feature_names=X_ok.columns.tolist()
    #                     )
    #                 plt.figure(figsize=(6, 4))
    #                 shap.plots.waterfall(exp, max_display=10, show=False)
    #                 plt.title(f"{target} Waterfall ({label.capitalize()} case)")
    #                 plt.tight_layout()
    ...
    #                 plt.close()
    #             print("📊 Waterfall plots saved (high / median / low)")
