...
__all__ = (
    ...
    'sludge_MT_conc', 'sludge_GT_conc', 'default_init_conds',
    'Q_sludge_GT', 'Q_sludge_MT', 'Temp', 
    )
...
Q_sludge_GT = 89.34 # m3/day
Q_sludge_MT = 45.27 # m3/day
...
# sready state concentrations
sludge_MT_conc = {'S_I': 1.104e+01, 
                  'S_S': 5.092e-01, 
                  'X_I': 7.888e+03, 
                  'X_S': 4.824e+02, 
                  'X_BH': 4.522e+04, 
...
def create_system(flowsheet=None, inf_kwargs={}, asm_kwargs={}, init_conds={}, lifetime=100, discount_rate=0.1,
    ...
    sludge_GT = qs.WasteStream('sludge_GT', T=Temp)
    sludge_MT = qs.WasteStream('sludge_MT', T=Temp)
    
    # inf_kwargs = inf_kwargs or default_inf_kwargs
    sludge_GT.set_flow_by_concentration(Q_sludge_GT, 
                                     concentrations= sludge_GT_conc, 
                                     units=('m3/d', 'mg/L'))
    
    sludge_MT.set_flow_by_concentration(Q_sludge_MT, 
                                     concentrations= sludge_MT_conc, 
                                      units=('m3/d', 'mg/L'))
    
    Mixer = su.Mixer('Mixer', [sludge_GT, sludge_MT])
    ...
    DG = UASB(ID='DG', ins = J1.outs[0], outs= ['gas', 'sludge_DG'], 
                          model=adm1, thermo = thermo_adm1, V_liq = 3217, V_gas = 321.7, pH_ctrl = 6.8, fraction_retain=0) 
    ...
    DU = su.Centrifuge('DU', ins = J2.outs[0], outs = ['sludge_DU', 'effluent_DU'], thermo = thermo_asm1,
                       thickener_perc=23, TSS_removal_perc=95)

    sys = qs.System('B1_ASM1', 
                    path=(Mixer, J1, DG, J2, DU), 
...
if __name__ == '__main__':
    ...
    # V_reactor = 1580*3.66 
    # numerator = mlss_reactor*V_reactor # mg/L * m3
    
    # denominator = (fs.WAS.get_TSS()*fs.WAS.F_vol) + (fs.effluent.get_TSS()*fs.effluent.F_vol) # mg/L * m3/hr
    
    # cal_SRT = (numerator/denominator) # in hr
    
    # print(f'cal_SRT  = {cal_SRT} hr')
...
#     print(f'GHG emissions = {CO2_eq_WRRF} kg CO2 eq/m3\n')
    ...
    fig, axis = fs.sludge_DU.scope.plot_time_series(('S_S', 'X_S', 'S_NH', 'S_NO', 'X_I')) 
    fig