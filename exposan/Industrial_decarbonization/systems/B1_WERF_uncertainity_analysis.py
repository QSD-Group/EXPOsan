...
def batch_init(sys, path, sheet):
    ...
    for k in sys.units:
        if sheet.startswith('ss'):
            ...
            # if k.ID.startswith('O1'):
            #     k.set_init_conc(**dct['O1'])
            # if k.ID.startswith('O2'):
            #     k.set_init_conc(**dct['O1'])
            # if k.ID.startswith('O3'):
            #     k.set_init_conc(**dct['O3'])
            # if k.ID.startswith('O4'):
            #     k.set_init_conc(**dct['O4'])
            # if k.ID.startswith('O5'):
            #     k.set_init_conc(**dct['O5'])
            # if k.ID.startswith('O6'):
            #     k.set_init_conc(**dct['O6'])
...
def create_system(flowsheet=None, inf_kwargs={}, asm_kwargs={}, init_conds={}, lifetime=100, discount_rate=0.1,
  ...
  # 'O1': 98.96749331543272}
    ...
    # PLEASE RUN THIS CONFIGURATION ON THE METRO BRANCH
    ...
    O1.natural_gas_price = 0.0041
    O1.CH4_natural_gas = 0.95
...
def run(t, method=None, **kwargs):
    ...
    # batch_init(sys, "/Users/saumitrarai/Dropbox/Personal/Gates_WWTP modelling/initial_conditions.xlsx", sheet='B1')
...
#               kind = 'coupled', # try 'isolated' too
#               units = 'USD/m3', 
#               baseline = fu.O1.natural_gas_price, 
#               distribution =  dist_natural_gas_price)
#     def set_dist_natural_gas_price(i):
#           fu.O1.natural_gas_price = i
          
#     dist_CH4_natural_gas =  shape.Uniform(lower= 0.90, upper=0.97) 
#     @param(name = 'Volumetric percentage of methane in natural gas', 
#               element = fu.O1, 
#               kind = 'coupled', # try 'isolated' too
#               units = 'percentage', 
#               baseline = fu.O1.CH4_natural_gas, 
#               distribution =  dist_CH4_natural_gas)
#     def set_dist_CH4_natural_gas(i):
#           fu.O1.CH4_natural_gas = i
...
#     @metric(name='Natural gas equivalent', units='USD/m3', element= fu.O1)
#     def natural_gas_costs():
#         return get_eq_natural_gas_price(system = sys, gas = fs.gas, 
#                                         natural_gas_price = fu.O1.natural_gas_price, 
#                                         CH4_nat_gas = fu.O1.CH4_natural_gas)
...
#         return get_CH4_CO2_eq_treatment(system = sys, influent_sc =influent_ST, effluent_sc = effluent_ST, effluent_sys =effluent, 
#                                   active_unit_IDs=act_power_units, sludge=disposed_sludge, 
...
#     samples = B1_model.sample(N=1000, rule='L')
...
#     B1_model.table.iloc[:,-13:] = all_outputs
    
#     B1_model.table

#     import pandas as pd
#     file_path = "/Users/saumitrarai/Dropbox/Rai-Guest_Shared/METAB_deployment_paper/Methods/Scenarios/B1/B1_comprehensive_uncertainty_analysis.xlsx"  
#     sheet_name = "Baseline_METAB_1000"  # Change this to your desired sheet name
