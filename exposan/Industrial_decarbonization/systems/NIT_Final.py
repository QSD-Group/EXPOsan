...
V_ae = 9084/6 # (2.4 MG) aerated zone tank volume [m3] (WERF report, F1) 

Q_was =  0.0132*(39290) # [m3/day] 0.1 MGD (WERF report, F1)
Q_ras =  0.67*(39290)  # [m3/day] 67% of effluent from PC (WERF report) # RERUN SIMULATION FOR F1 TO SEE WHAT THIS NUMBER IS
...
def create_system(flowsheet=None, inf_kwargs={}, asm_kwargs={}, init_conds={}, lifetime=100, discount_rate=0.1,
    ...
    #                   TSS_removal_perc=70, thermo = thermo_asm2d)
    ...
    # thickener_perc and TSS_removal_perc based on WERF report (F1)
    ...
    # Volume of AD based on WERF report (F1)
    ...
    # thickener_perc and TSS_removal_perc based on WERF report (F1)1
    ...
    DU.CH4_EF_st = 0.0075 
    DU.N2O_EF_st = 0.004 # (Jason)
    DU.CH4_EF_dis = 0.009
    # DU.N2O_EF_dis = 0.005
    DU.nat_gas_EF = 0.454
   

    sys = qs.System('F1_WERF', 
                    path=(PC, 
                           O1, O2, O3, O4, O5, O6, 
...
def run(t, method=None, **kwargs):
    ...
    batch_init(sys, "/Users/saumitrarai/Desktop/Research/MS_research/metro/data/initial_conditions_ASM2d.xlsx", sheet='t=10')
...
# # # # ------------------------------------------------------------------------------------------------------------------
    ...
    # fig, axis = fs.effluent.scope.plot_time_series(('S_A', 'S_F', 'X_S', 'S_NH', 'S_NO', 'X_I', 'S_I', 'S_N2')) 
    # fig
...
#     @metric(name='Total operational cost', units='USD/m3', element= fu.O1)
...
#     @metric(name='Natural gas cost', units='USD/m3', element= fu.O1)
#     def nat_gas_cost():
#         return get_eq_natural_gas_price(system = sys, gas = fs.gas, 
#                                         mass_density = 0.68, 
#                                         natural_gas_price = fu.O1.nat_gas_price)
...
#     samples = F1_model.sample(N=10000, rule='L')
...
#     import pandas as pd
#     file_path = "/Users/saumitrarai/Desktop/Research/MS_research/metro/results/F1_comprehensive_uncertainty_analysis.xlsx"  
