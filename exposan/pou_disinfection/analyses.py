...
from exposan.POU_dis import results_path, figures_path, evaluate, get_key_metrics
from exposan.POU_dis.models import organize_uncertainty_results, save_uncertainty_results

# Comment these out if want to see all warnings
import warnings
warnings.filterwarnings(action='ignore')

models = modelD, modelE = pou.modelD, pou.modelE
RGBs = {
...
param_set = set(modelD.parameters), set(modelE.parameters)
...
def run(N_uncertainty=5000, N_morris=50, from_record=True,
    ...
    for model in models:
        ...
        if not from_record:
            ...
            if model is modelD:
                copy_samples(modelE, model)
            elif model is modelE:
                copy_samples(modelE, model)
