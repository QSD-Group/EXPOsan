...
SE = su.DynamicInfluent('SE', outs=[DYINF],
                        data_file=ospath.join(data_path, 'dynamic_influent.tsv'))
...
batch_init(ospath.join(data_path, 'initial_conditions_pm2_dynamic_influent.xlsx'), 'default')
...
def run(t, t_step, method=None, print_t=False, **kwargs):
    if method:
        eco.simulate(state_reset_hook='reset_cache',
                      ...
                      # rtol=1e-2,
                      # atol=1e-3,
                      export_state_to=ospath.join(results_path, f'sol_{t}d_{method}_ecorecover_lca.xlsx'),
                      print_t=print_t,
                      **kwargs)
    else:
        eco.simulate(state_reset_hook='reset_cache',
