...
from exposan.pm2_fourty import data_path
...
T_pbr, I_pbr = EDV.batch_init(os.path.join(data_path, 'exo_vars_dynamic_influent_revised_fourty_cali.xlsx'), 'linear')
...
# def create_system(flowsheet=None, inf_kwargs={}, pm2_kwargs={}, init_conds={}):
    ...
    flowsheet = flowsheet or qs.Flowsheet('pm2_fourty')
    ...
    SE = su.DynamicInfluent('SE', outs=[DYINF],
                            data_file=os.path.join(data_path, 'dynamic_influent_revised_fourty_cali.tsv'))
    # SE = su.DynamicInfluent('SE', outs=[DYINF],
    ...
    batch_init(sys,
               os.path.join(data_path, 'initial_conditions_pm2_dynamic_influent_revised_fourty_cali.xlsx'), 'default')
