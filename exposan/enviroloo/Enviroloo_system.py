...
Q_ras = 11.67 # m3/h # 200 l/min for nitrate pump
# Q_was = 280 # m3/day # 200 l/min
toilet_waste={'S_NH4':  13.7,
              'S_NO3': 0.2,     # Nitrate
                'S_PO4': 2.3,
                'S_O2': 3.0,      # Dissolved oxygen
                'S_F': 19.1,      # Fermentable COD
                'S_I': 12.8,      # Inert soluble COD
                'X_S': 22,         # Slowly biodegradable particulates = TSS
                'S_IC':   0.148,
                'S_K':   0.0694,
                'S_Mg':   0.00833,
                'S_Ca':   0.0117,
...
def create_systemEL(flowsheet=None, inf_kwargs={}, masm_kwargs={}, init_conds={},
    ...
    sludge_MembT = qs.WasteStream('sludge_MT', T=Temp)
    sludge_MT_PC = qs.WasteStream('sludge_MT_PC', T=Temp)
    sludge_MT_A1 = qs.WasteStream('sludge_MT_A1', T=Temp)
    flushing_water_CT = qs.WasteStream('flushing_water_CT', T=Temp)
    WAS = qs.WasteStream('WAS', T=Temp)
    
    toilet_ins.set_flow_by_concentration(Q_w, 
                                      concentrations=toilet_waste, 
                                      units=('m3/d', 'mg/L'))
    
    
    masm_kwargs = masm_kwargs or default_masm2d_kwargs
    ...
    CT = su.Mixer('CT', ins=(toilet_ins, sludge_PC, flushing_water_CT, 'spill_PC'), 
                    outs = ('effluent_CT'),
                    isdynamic=False,
    ...
    PC = elu.EL_PC('PC', ins=(CT-0, sludge_MT_PC), outs=('effluent_PC_total', sludge_PC),
               ppl=ppl, baseline_ppl=100,
               solids_removal_efficiency=0.85,
               isdynamic=True,
               thermo=thermo_masm2d,
               sludge_flow_rate=36)  # 0.00116 m3/hr
    ...
    A1 = elu.EL_Anoxic('A1', ins=(S3-0, sludge_MT_A1), 
                            outs = ('effluent_AnoxT'), isdynamic=True, **kwargs_1
                            # W_tank= 2.09,
                            # # ppl = ppl, baseline_ppl = 100,
                            # aeration=None, DO_ID='S_O2', suspended_growth_model=asm2d, 
    ...
    B1 = elu.EL_CMMBR('B1', ins=(O1-0), 
                        outs = (sludge_MembT, 'effluent_MembT'),
                        # DO_ID='S_O2', aeration=2, suspended_growth_model=asm2d,
    ...
    # breakpoint()
    S2 = su.Splitter('S2', ins = B1-0, outs = ['sludge_MT', WAS], split= 0.8)
    
    # # S2.run()
    
    S1 = su.Splitter('S1', ins = S2-0, outs = [sludge_MT_PC, sludge_MT_A1], split= 0.5)
    ...
    S4 = su.Splitter('S4', ins = CWT-0, outs = [flushing_water_CT, 'Reflushing'], split= 0.5)
