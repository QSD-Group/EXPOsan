...
from exposan.pm2_ecorecover_ai_2nd import data_path
...
T_pbr, I_pbr = EDV.batch_init(os.path.join(data_path, 'exo_vars_dynamic_influent_cali2.xlsx'), 'linear')
...
def create_system(flowsheet=None, pm2_kwargs={}, init_conds={}):
    ...
    flowsheet = flowsheet or qs.Flowsheet('pm2_ecorecover_ai_2nd')
    ...
    SE = su.DynamicInfluent('SE', outs=[DYINF],
                            data_file=os.path.join(data_path, 'dynamic_influent_cali2.tsv'))
    ...
    batch_init(sys,
               os.path.join(data_path, 'initial_conditions_pm2_dynamic_influent_cali2.xlsx'), 'default')
