...
from exposan.pm2_ecorecover_ai_2nd import (
    data_path,
    results_path,
    )
...
Temp = 287.19          # temperature [K]
# Temp = 286.08          # temperature [K]

T, I = EDV.batch_init(os.path.join(data_path, 'exo_vars_dynamic_influent_vali2.xlsx'), 'linear')
...
pm2 = pc.PM2(arr_e=6957.51421631991, K_P=77.4477922064014, f_CH_max=6.74589415166239, exponent=4.85674194210288,
             q_CH=1.88934264277591, q_LI=32.706759306568, V_NH=0.0302136600989035, V_P=0.31214778480535,
             ...
             Y_LI_NR_HET_GLU=1.620, Y_LI_ND_HET_GLU=1.046, Y_X_ALG_HET_GLU=0.317)  # ecorecover_cali (optuna results) seed777_simultaneous calibration
...
SE = su.DynamicInfluent('SE', outs=[DYINF],
                        data_file=ospath.join(data_path, 'dynamic_influent_vali2.tsv'))
...
PBR19 = su.CSTR('PBR19', ins=PBR18-0, V_max=V_pbr/20,
              aeration=None, suspended_growth_model=pm2, exogenous_vars=(T, I))
PBR20 = su.CSTR('PBR20', ins=PBR19-0, outs=[ME, INT], split=[0.92, 0.08], V_max=V_pbr/20,
              aeration=None, suspended_growth_model=pm2, exogenous_vars=(T, I))

MEM = su.Splitter('MEM', PBR20-0, outs=[TE, RETEN], split=0.42*(1-cmps.x))
...
batch_init(ospath.join(data_path, 'initial_conditions_pm2_dynamic_influent_vali2.xlsx'), 'default')
...
def run(t, t_step, method=None, print_t=False, **kwargs):
    if method:
        eco.simulate(state_reset_hook='reset_cache',
                      ...
                      # rtol=1e-2,
                      # atol=1e-3,
                      export_state_to=ospath.join(results_path, f'sol_{t}d_{method}_validation2_result.xlsx'),
                      print_t=print_t,
                      **kwargs)
    else:
        eco.simulate(state_reset_hook='reset_cache',
