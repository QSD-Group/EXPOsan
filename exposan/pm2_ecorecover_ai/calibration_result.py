...
pm2 = pc.PM2(arr_e=6622.86679509832, K_P=29.2038598164409, f_CH_max=4.15621171464863, exponent=1.00773141919451,
             q_CH=0.1011737284021, q_LI=30.3652465518411, V_NH=0.317342203660257, V_P=0.294473406804999,
...
def run(t, t_step, method=None, print_t=False, **kwargs):
    if method:
        eco.simulate(state_reset_hook='reset_cache',
                      ...
                      # rtol=1e-2,
                      # atol=1e-3,
                      export_state_to=ospath.join(results_path, f'sol_{t}d_{method}_calibration_result_test122025.xlsx'),
                      print_t=print_t,
                      **kwargs)
    else:
        eco.simulate(state_reset_hook='reset_cache',
