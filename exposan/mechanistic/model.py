...
from exposan.mechanistic import (
    biomass_IDs,
    create_system,
    default_init_conds,
    results_path,
...
def create_model(system=None, flowsheet=None, kind='general'):
    ...
    effluent, WAS = stream.effluent, stream.WAS
    # wastewater, effluent, WAS = stream.wastewater, stream.effluent, stream.WAS

    cmps = effluent.components

    unit = sys.flowsheet.unit
    ANO, FAC, AER1, AER2, AER3, C1 = unit.ANO, unit.FAC, unit.AER1, unit.AER2, unit.AER3, unit.C1
    aer = AER1.aeration
    ...
    if kind == 'general':
        ...
        # @param(name='Anoxic zone hydraulic retention time', element=ANO,
        ...
        for name, vals in param_ranges.items():
            ...
            param(setter=setter, name=f'ASM1 {name}', element=ANO, kind='coupled',
        ...
        @param(name='ASM1 f_Pobs', element=ANO, kind='coupled', units='',
               baseline=b, distribution=D)
        def set_f_Pobs(i):
        ...
        @param(name='KLa', element=AER1, kind='coupled', units='',
               baseline=b, distribution=D)
        def set_O1_O2_KLa(i):
            aer.KLa = i
        ...
        # b = aer.KLa
        # D = get_uniform_w_frac(b, 0.1)
        # @param(name='O3 KLa', element=AER3, kind='coupled', units='',
        #        baseline=b, distribution=D)
        # def set_O3_KLa(i):
        #     aer.KLa = i
        ...
        b = 8.0
        D = get_uniform_w_frac(b, 0.1)
        @param(name='Saturation DO', element=AER1, kind='coupled', units='mg/L',
               baseline=b, distribution=D)
        def set_DOsat(i):
            aer.DOsat = i
    ...
    ##### UA with random initial conditions to test steady state #####
        ...
        for k, v in _ic.items():
            ...
            @param(name='initial '+k, element=ANO, kind='coupled', units='mg/L',
                      baseline=b, distribution=D)
    ...
    ##### UUA with 2 decision variables as parameters #####
        ...
        @param(name='O1 O2 air flowrate', element=AER1, kind='coupled', units='m3/d',
               baseline=b, distribution=D)
        def set_Q_air(i):
            aer.Q_air = i
