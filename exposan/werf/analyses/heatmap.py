...
def heatmap(data, colorbar=True, cmap='viridis', 
            ...
            save_as=None, **kwargs):
    fig, ax = plt.subplots(figsize=(10, data.shape[0]*0.8))
    ...
    fig.subplots_adjust(bottom=0.12, top=0.98, left=0.075, right=0.985)
...
def plot_absolute(data=None, suffix='baseline_unopt'):
    ...
    heatmap(fill.loc[eff_vars,:], annotate=vals.loc[eff_vars,:], valfmt=valfmtwnan, 
            txtcolors=txtcolors, save_as=f'eff_{suffix}.tif',
    ...
    heatmap(fill.loc[op_vars,:], 
            ...
            txtcolors=txtcolors, save_as=f'op_{suffix}.tif')
...
def plot_opex_diff(data=None):
    ...
    fig.savefig(ospath.join(figures_path, 'dopex.tif'), 
                dpi=300, transparent=True)

#%%
if __name__ == '__main__':
    df_baseline_opt = load_data(
        ospath.join(results_path, 'baseline_opt_performance.xlsx'), sheet=0,
        header=[0,1], skiprows=[2,]
        )
