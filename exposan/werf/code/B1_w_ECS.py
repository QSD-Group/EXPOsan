...
def create_b1_system(flowsheet=None, default_init_conds=True):
    ...
    ASR.temperature = 20
    ASR.blower_efficiency = 0.7
    ASR.inlet_pressure_loss = 1
    ASR.diffuser_pressure_loss = 7
    ASR.sludge_disposal_cost = 375
    ASR.unit_electricity_costs = 0.0577
    ASR.nat_gas_price = 0.1617 # USD/m3
    ...
    if ESAP_inc == 'included':
        ...
        ESAP = qs.sanunits.ESAP('ESAP', ins = AD-1, component_ID_NH3 = 'S_IN', component_ID_P ='S_IP', component_ID_Mg = "S_Mg", outs=('recovered_product','loss','ESAP_effluent'),
    ...
    DW.DOC_f = 0.45
    DW.MCF = 0.8
    DW.k = 0.06 # temperate and boreal (dry)
    DW.pl = 30
    DW.elec_EF = 0.675 # the value for Minnesota
    DW.CH4_EF_st = 0.0075 
    DW.N2O_EF_st = 0.004 # Jason Ren
    DW.CH4_EF_dis = 0.009
    # DW.N2O_EF_dis = 0.005
    DW.nat_gas_EF = 0.454
    ...
    if ESAP_inc == 'included':
        sys = qs.System(
            ID, 
            path=(PC, GT, ASR, FC, MT, M1, J1, AD, ESAP, J2, DW, M2, HD),
...
#             baseline = fu.ASR.temperature, # mean of upper and lower limits
#             distribution = dist_T)
#     def set_air_temperature(i):
#         fu.ASR.temperature = i
...
#             kind = 'coupled', # try 'isolated' too
#             units = 'unitless', 
#             baseline = fu.ASR.blower_efficiency, # mean of upper and lower limits
#             distribution = dist_blower_efficiency)
#     def set_blower_efficiency(i):
#         fu.ASR.blower_efficiency = i
...
#             kind = 'coupled', # try 'isolated' too
#             units = 'kPa', 
#             baseline = fu.ASR.inlet_pressure_loss, # mean of upper and lower limits
#             distribution = dist_P_inlet_loss)
#     def set_inlet_pressure_loss(i):
#         fu.ASR.inlet_pressure_loss = i
...
#             kind = 'coupled', # try 'isolated' too
#             units = 'kPa', 
#             baseline = fu.ASR.diffuser_pressure_loss, # mean of upper and lower limits
#             distribution = dist_P_diffuser_loss)
#     def set_diffuser_pressure_loss(i):
#         fu.ASR.diffuser_pressure_loss = i
...
#             kind = 'coupled', # try 'isolated' too
#             units = 'USD/ton', 
#             baseline = fu.ASR.sludge_disposal_cost, # mean of upper and lower limits
#             distribution = dist_disposal_costs)
#     def set_sludge_disposal_cost(i):
#         fu.ASR.sludge_disposal_cost = i
...
#             kind = 'coupled', # try 'isolated' too
#             units = 'USD/kWh', 
#             baseline = fu.ASR.unit_electricity_costs, # mean of upper and lower limits
#             distribution = dist_electricity_cost)
#     def set_electricity_cost(i):
#         fu.ASR.unit_electricity_costs = i
...
#             kind = 'coupled', # try 'isolated' too
#             units = 'unitless', 
#             baseline = fu.DW.DOC_f, # Deafault = 0.38 (Table 2.4 V5 Ch2, IPCC 2019)
...
#               kind = 'coupled', # try 'isolated' too
#               units = 'unitless', 
#               baseline = fu.DW.MCF, # Table 3.1, V5 Ch3, IPCC 2019
...
#     dist_elec_EF = shape.Uniform(lower = 0.244, upper = 0.814) # in kg-CO2-Eq/kWh
#     @param(name = 'EF for electrcity', 
#               element = fu.DW, 
#               kind = 'coupled', # try 'isolated' too
#               units = 'kg-CO2-Eq/kWh', 
#               baseline = fu.DW.elec_EF, 
...
#               baseline = fu.DW.CH4_EF_st, 
#               distribution =  dist_CH4_EF_st)
#     def set_CH4_EF_st(i):
#           fu.DW.CH4_EF_st = i
         
#     # N2O_EF_sc : float, optional
#     #     The emission factor used to calculate nitrous oxide emissions in secondary treatment. The default is 0.004 kg N2O-N/ kg N. (Cuihong Song and Jason Ren)
#     dist_N2O_EF_st = shape.Triangle(lower= 0.000044, midpoint=0.0040, upper=0.0172) # in kg N2O-N/ kg N (used Song et al. 2024 SI to find all WRRFs with bioreactor as the filter)
#     @param(name = 'EF for N2O during treatment', 
#               element = fu.DW, 
#               kind = 'coupled', # try 'isolated' too
#               units = 'kg N/kg influent N', 
#               baseline = fu.DW.N2O_EF_st, 
...
#     @metric(name='Total operational cost', units='USD/m3', element= fu.ASR)
...
#                                               T= fu.ASR.temperature, 
#                                               efficiency= fu.ASR.blower_efficiency, 
#                                               P_inlet_loss= fu.ASR.inlet_pressure_loss, 
#                                               P_diffuser_loss= fu.ASR.diffuser_pressure_loss, 
                                              
#                                               unit_weight_disposal_cost = fu.ASR.sludge_disposal_cost,
#                                               unit_electricity_cost = fu.ASR.unit_electricity_costs)
    
#     @metric(name='Aeration cost', units='USD/m3', element= fu.ASR)
#     def aeration_cost():
...
#                               unit_electricity_cost = fu.ASR.unit_electricity_costs)
#     act_power_units
#     @metric(name='Pumping cost', units='USD/m3', element= fu.ASR)
#     def pumping_cost():
...
#     @metric(name='Sludge disposal cost', units='USD/m3', element= fu.ASR)
#     def sludge_disposal_costs():
#         return get_sludge_disposal_costs(sludge = disposed_sludge,  # sludge disposal costs 
#                                       system= sys, # uncertain parameters 
#                                       unit_weight_disposal_cost = fu.ASR.sludge_disposal_cost # sludge disposal costs 
#                                       )
    
#     @metric(name='Natural gas cost', units='USD/m3', element= fu.ASR)
#     def nat_gas_cost():
#         return get_eq_natural_gas_price(system = sys, gas = fs.biogas, 
#                                         mass_density = 0.68, 
#                                         natural_gas_price = fu.ASR.nat_gas_price)
    
#     @metric(name='Total GHG emissions', units='kg CO2 eq./m3', element= fu.DW)
#     def GHG_emissions():
...
#                                 CH4_EF_sc = fu.DW.CH4_EF_st, 
#                                 N2O_EF_sc = fu.DW.N2O_EF_st, 
#                                 CH4_EF_discharge = fu.DW.CH4_EF_dis,
#                                 N2O_EF_discharge = 0.005,
                                 
#                                 CO2_EF= fu.DW.elec_EF, 
#                                 DOC_f = fu.DW.DOC_f, 
#                                 MCF = fu.DW.MCF, 
#                                 k = fu.DW.k, 
#                                 pl= fu.DW.pl)
    
#     @metric(name='CH4 CO2_eq_treatment', units='kg CO2 eq./m3', element= fu.DW)
#     def CH4_CO2_eq_treatment():
#         return get_CH4_CO2_eq_treatment(system = sys, influent_sc =influent_ST, effluent_sc = effluent_ST,
#                                   # uncertain parameters 
#                                 CH4_EF_sc = fu.DW.CH4_EF_st)
        
#     @metric(name='N2O CO2_eq_treatment', units='kg CO2 eq./m3', element= fu.DW)
#     def N2O_CO2_eq_treatment():
#         return get_N2O_CO2_eq_treatment(system = sys, influent_sc =influent_ST,  
#                                   N2O_EF_sc=fu.DW.N2O_EF_st)
        
#     @metric(name='CH4 CO2_eq_discharge', units='kg CO2 eq./m3', element= fu.DW)
#     def CH4_CO2_eq_discharge():
#         return get_CH4_CO2_eq_discharge(system = sys, effluent_sys = effluent, 
#                                   CH4_EF_discharge=fu.DW.CH4_EF_dis)
    
#     @metric(name='N2O CO2_eq_discharge', units='kg CO2 eq./m3', element= fu.DW)
#     def N2O_CO2_eq_discharge():
#         return get_N2O_CO2_eq_discharge(system = sys, effluent_sys = effluent,
#                                         N2O_EF_discharge  = 0.005)
    
#     @metric(name='CH4 emitted_during_pl', units='kg CO2 eq./m3', element= fu.DW)
...
#     @metric(name='CH4 emitted_after_pl', units='kg CO2 eq./m3', element= fu.DW)
...
#                               pl= fu.DW.pl
#                               )
#     @metric(name='CO2 eq_electricity', units='kg CO2 eq./m3', element= fu.DW)
...
#         for p, v in zip(B1_model.parameters, sample):
#             p.setter(v)
    
#         out = [m() for m in B1_model.metrics]
#         all_outputs.append(out)
    
#     B1_model.table.iloc[:,-14:] = all_outputs
...
#     if ESAP_inc == 'included':
#         sheet_name = "B1_with_ECS"  
#     elif ESAP_inc == 'not_included':
#         sheet_name = "B1" 
    
#     with pd.ExcelWriter(file_path, engine='openpyxl', mode='a') as writer:
#         B1_model.table.to_excel(writer, sheet_name=sheet_name)
    
#     fig, ax = qs.stats.plot_uncertainties(B1_model, x_axis=B1_model.metrics[0], y_axis=B1_model.metrics[1],
#                                       kind='kde-kde', center_kws={'fill': True})
#     fig

#     r_df, p_df = qs.stats.get_correlations(B1_model, kind='Spearman')
#     print(r_df, p_df)
