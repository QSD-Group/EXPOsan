...
from biosteam import System, Flowsheet
# import numpy as np
...
def create_system(flowsheet_name='', set_init=True):
    f = Flowsheet(flowsheet_name)
...
def run(t, method, set_init, **kwargs):
    bsm1 = create_system(flowsheet_name=method, set_init=set_init).system.BSM1
    bsm1.simulate(t_span = (0, t), method=method)

#!!! Can't figure out a way to reset units, so had to recreate the system for batch-running
# def reset_system():
#     for u in bsm1.units:
#         for s in u.outs:
#             s.empty()
