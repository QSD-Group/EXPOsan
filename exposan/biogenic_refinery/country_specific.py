...
def create_country_specific_model(ID, country):
    ...
    # Operator daily wage
    b = country_dct['operator_daily_wage']
    ...
    b = country_dct['certified_electrician_wages']
    D = shape.Uniform(lower=(b * 0.5), upper=(b * 1.5))
    @param(name='Certified electrician wages',
    ...
    # Service team wage
    b = country_dct['service_team_wages']
    D = shape.Uniform(lower=(b * 0.5), upper=(b * 1.5))
    @param(name='Service team wages',
           element=excretion_unit,  # just want to add to the 0th unit of the system
           kind='coupled', units='USD/h',
           baseline=b, distribution=D)
    ...
    b = country_dct['facility_manager_wages']
    D = shape.Uniform(lower=(b * 0.5), upper=(b * 1.5))
    @param(name='Facility manager wages',
           element=excretion_unit,  # just want to add to the 0th unit of the system
           kind='coupled', units='USD/h',
           baseline=b, distribution=D)
    ...
    b = country_dct['biomass_controls_wages']
    D = shape.Uniform(lower=(b * 0.5), upper=(b * 1.5))
    @param(name='Biomass controls wages',
           element=excretion_unit,  # just want to add to the 0th unit of the system
           kind='coupled', units='USD/h',
           baseline=b, distribution=D)
    ...
    b = country_dct['struvite_fertilizer_price']
    D = shape.Uniform(lower=(b * 0.8), upper=(b * 1.2))
...
def run_country(input_dct, system_IDs=(), folder_path='', seed=None, N=1000):
    ...
    for country, country_dct in input_dct.items():
        for sys_ID in system_IDs:
            ...
            path = os.path.join(folder_path, f'br_{country}_{sys_ID}.xlsx')
