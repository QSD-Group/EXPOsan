...
# -*- coding: utf-8 -*-
"""
Created on Wed Jan 26 00:29:16 2022
...
def run_country(dct):
    ...
    for country, country_dct in dct.items():
        ...
        data = load_data(path)
        m.batch_setting_unit_params(data, modelB, B1, exclude=('e_cal','p_anim','p_veg')) #adds distributions to tsv file
        ...
        data = load_data(path)
        m.batch_setting_unit_params(data, modelB, B6)
        ...
        data = load_data(path)
        m.batch_setting_unit_params(data, modelB, B8)
        
        # Housing biogenic refinery
        B9 = systems.B9
        
        # construction
        B9.const_wage = country_dct['construction']
        
        path = su_data_path + '_housing_biogenic_refinery.tsv'
        data = load_data(path)
        m.batch_setting_unit_params(data, modelB, B9)
        ...
        data = load_data(path)
        m.batch_setting_unit_params(data, modelB, B12)
        
        # Oil heat exchanger
        B13 = systems.B13
        path = su_data_path + '_oil_heat_exchanger.tsv'
        data = load_data(path)
        m.batch_setting_unit_params(data, modelB, B13)
        
        # Hydronic heat exchanger
        B14 = systems.B14
        path = su_data_path + '_hydronic_heat_exchanger.tsv'
        data = load_data(path)
        m.batch_setting_unit_params(data, modelB, B14)
        
        # Dryer from HHx
        B15 = systems.B15
        path = su_data_path + '_dryer_from_hhx.tsv'
        data = load_data(path)
        m.batch_setting_unit_params(data, modelB, B15)


        results[country] = m.run_uncertainty(model=modelB, seed=5, N=1000) 
        del sysB, modelB
...
def save_uncertainty_results(results):
    ...
    for country, dct in results.items():
        file_name = path+'/'+country+'_B'+'.xlsx'
