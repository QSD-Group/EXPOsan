...
dct = globals()
for unit_alias in (
        'WWTP', 'SluC', 'P1', 'H1', 'HTL', 'HTL_hx', 'HTL_drum', 'H2SO4_Tank',
...
sys_no_P = qs.System('sys_no_P', path=(WWTP, SluC, P1, H1, HTL, H2SO4_Tank,
...
sys_no_P.operating_hours = WWTP.operation_hours # 7920 hr Jones

lca_diesel = qs.LCA(system=sys_no_P, lifetime=30, lifetime_unit='yr',
                    Electricity=lambda:(sys_no_P.get_electricity_consumption()-sys_no_P.get_electricity_production())*30,
                    Cooling=lambda:sys_no_P.get_cooling_duty()/1000*30)
...
lca_sludge = qs.LCA(system=sys_no_P, lifetime=30, lifetime_unit='yr',
              Electricity=lambda:(sys_no_P.get_electricity_consumption()-sys_no_P.get_electricity_production())*30,
              Cooling=lambda:sys_no_P.get_cooling_duty()/1000*30)

tea = create_tea(sys_no_P)

# return sys_no_P

#%%
model = Model(sys_no_P)
...
def set_operation_hour(i):
    WWTP.operation_hours=sys_no_P.operating_hours=i
...
def get_CAPEX():
    return sys_no_P.installed_equipment_cost
...
def get_Phosphorus_CAPEX():
    return StruPre.installed_cost+H2SO4_Tank.installed_cost*SP1.outs[0].F_mass/SP1.F_mass_out
...
def get_material_VOC():
    return sys_no_P.material_cost

@metric(name='H2SO4_VOC',units='$/yr',element='TEA')
def get_H2SO4_VOC():
    return H2SO4_Tank.ins[0].price*H2SO4_Tank.ins[0].F_mass*sys_no_P.operating_hours

@metric(name='H2_VOC',units='$/yr',element='TEA')
def get_H2_VOC():
    return RSP1.ins[0].price*RSP1.ins[0].F_mass*sys_no_P.operating_hours

@metric(name='CHG_catalyst_VOC',units='$/yr',element='TEA')
def get_CHG_catalyst_VOC():
    return CHG.ins[1].price*CHG.ins[1].F_mass*sys_no_P.operating_hours

@metric(name='MgCl2_VOC',units='$/yr',element='TEA')
def get_MgCl2_VOC():
    return StruPre.ins[1].price*StruPre.ins[1].F_mass*sys_no_P.operating_hours

@metric(name='other_materials_VOC',units='$/yr',element='TEA')
def get_other_materials_VOC():
    a = 0 
    for i in range (max(len(unit.ins) for unit in sys_no_P.units)):
        for unit in sys_no_P.units:
            if len(unit.ins) == i:
                for j in range (i):
                    if unit.ins[j].price*unit.ins[j].F_mass*sys_no_P.operating_hours != 0:
                        a+=unit.ins[j].price*unit.ins[j].F_mass*sys_no_P.operating_hours
    a -= (H2SO4_Tank.ins[0].price*H2SO4_Tank.ins[0].F_mass +\
          RSP1.ins[0].price*RSP1.ins[0].F_mass +\
          CHG.ins[1].price*CHG.ins[1].F_mass +\
          StruPre.ins[1].price*StruPre.ins[1].F_mass)*sys_no_P.operating_hours
    return a
...
def get_LCA_diesel():
    return lca_diesel.get_total_impacts()['GlobalWarming']/FuelMixer.outs[0].F_mass/sys_no_P.operating_hours/lca_diesel.lifetime*_kg_to_g/45.5/_MJ_to_MMBTU
# diesel: 45.5 MJ/kg

@metric(name='GWP_sludge',units='kg CO2/ton dry sludge',element='LCA')
def get_LCA_sludge():
    return lca_sludge.get_total_impacts()['GlobalWarming']/raw_wastewater.F_vol/_m3perh_to_MGD/WWTP.ww_2_dry_sludge/(sys_no_P.operating_hours/24)/lca_sludge.lifetime
