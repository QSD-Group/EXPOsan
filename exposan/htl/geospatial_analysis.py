...
# we have a problem here, that is CRS 3857 is not accurate at all, especially more far away from the equator,
# therefore I am thinking to use a larger distance here, for example, 150000 (tested, when use 150000,
# all actual distances are smaller than 100000) as the max_distance and then use geopy.distance.geodesic to
# recalculate the distance and filter out those that are actually longer than 100000
...
# select out WWTPs that are within 100 km of refineries
...
# WWTP_within.to_excel(folder + 'HTL_spatial_model_input.xlsx') # this will be the input for the future analysis

#%%
# cumulative WRRFs capacity vs distances (Figure SX) #!!! confirm figure number
...
WWTP_within = pd.read_excel(folder + 'HTL_spatial_model_input.xlsx')
assert len(WWTP_within) == 449, 'check the length of input and separate into multiple consoles to speed up'
...
for i in range(0, 499):
    ...
    if WWTP_within.iloc[i]['solid_fate'] == 8: # lagoon and constructed wetland
        waste_price_value = 
        waste_GHG_value =
        
    elif WWTP_within.iloc[i]['solid_fate'] in (1, 2, 4): # anaerobic or aerobic digestion
        waste_price_value = 
        waste_GHG_value = 
        
    else: # dewatering only and dewatering+incineration
        waste_price_value = 
        waste_GHG_value = 
        
    sys, barrel = create_spatial_system(waste_price=waste_price_value,
    ...
    CO2_reduction_result = WWTP_within.iloc[i]['TOTAL SOLIDS TONS_DAY']*(1-WWTP_within.iloc[i]['reduction_factor_ave'])*7920/24*30*(waste_GHG_value-kg_CO2_per_ton_dry_sludge)
