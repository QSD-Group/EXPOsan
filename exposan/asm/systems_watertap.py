...
def run(process_model, aerated, t, t_step, method, simulate=True,
    ...
    if save_stoichiometry:
        ...
        asm.stoichiometry.to_csv(f'results_watertap/{process_model}_{suffix}_stoichiometry.csv')
    export_state_to = f'results_watertap/{process_model}_{suffix}.xlsx' if save_states else ''
    if simulate:
